...
# 1. Limpieza preventiva de nombres de columnas (Best Practice)
# Spark puede dar error si tus columnas en Pandas tienen espacios o caracteres raros
df_clean_1.columns = [c.replace(" ", "_").replace(".", "") for c in df_clean_1.columns]

# 2. Convertir a Spark
# Habilita Arrow para optimizar la conversión de Pandas a Spark (hace el proceso mucho más rápido)
spark.conf.set("spark.sql.execution.arrow.pyspark.enabled", "true")
sparkDF = spark.createDataFrame(df_clean_1) 

# 3. Escribir como Tabla Delta Gestionada
# Usamos saveAsTable para registro inmediato en el catálogo
sparkDF.write \
    .mode("overwrite") \
    .option("overwriteSchema", "true") \
    .format("delta") \
    .saveAsTable(table_name) # Nota: No hace falta poner "Tables/", solo el nombre

print(f"Spark dataframe saved (and registered) to delta table: {table_name}")
